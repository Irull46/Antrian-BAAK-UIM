...
!(function () {
    ...
    const comp = {
        txt: root.querySelector("#antrian"),
    ...
    const tts = new TextToSpeech({
        onVoicesChanged: () => {
            ...
            const selectedVoice = tts
                .getVoices()
                .find((voice) => voice.lang === "id-ID");
            if (selectedVoice) {
                voiceOpts.find(
                    (opt) => opt.getAttribute("value") === selectedVoice.name
                ).selected = true;
            }
        },
